...
export class SigninAuthenticationService extends CommonResponse {
    ...
    async signin(data: SigninDto, login_data: loginDataType): Promise<ResponseService> {
        const user: Partial<AuthUsers> = await this._signinUseCase.controlUser(data.email);
        await this._signinUseCase.controlPassword(data.password, user.credential.password);
  
        const permissions = this._signinUseCase.encryptDataToBeSent(user);

        const accessToken = await this._signinUseCase.generateToken(user, login_data)
