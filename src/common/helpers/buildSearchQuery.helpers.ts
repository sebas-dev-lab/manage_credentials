...
import { setPaginationOptions, setPaginationQueryOptions } from "../utils/setMetadataPagination.utils";
...
export default class BuildSearchQuery<T> {
    ...
    setTerm(): void {
        ...
        if (this.search.term && this.fieldsToSearchTerm.length) {
            for (const field of this.fieldsToSearchTerm) {
                this.whereClause += index === 0 ? ` AND (${field} ILIKE '%${this.search.term}%'` : index === this.fieldsToSearchTerm.length - 1 ? ` OR ${field} ILIKE '%${this.search.term}%')` : ` OR ${field} ILIKE '%${this.search.term}%'`
                index++;
            }
        }
    }
    ...
    private buildQueryByOperation(operation: 'select' | 'count', pageOptions?: string): string {

        const order_by = this.search?.order_by ? this.search?.order_by.split(';').map((or) => or.split(':')) : [['id', 'asc']]

        return `
        SELECT ${operation === 'count' ? 'count(*)' : '*'}
        FROM ${this.table_name}
        ${operation !== 'count' ? `WHERE ${this.whereClause}` : ''}
        ${operation !== 'count' ? `ORDER BY ${order_by.map(([field, order]) => `${field} ${order}`).join(', ')}` : ''}
